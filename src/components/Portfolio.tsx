...
function AiLab({ es: _es }: { es: boolean }) {
...
export function Portfolio({ projectContent }: { projectContent: Project[] }) {
  ...
  useLayoutEffect(() => {
    ...
    const savedLanguage = localStorage.getItem("studiowolfcode-language");
  ...
  const toggleLanguage = () => setLanguage((current) => {
    ...
    localStorage.setItem("studiowolfcode-language", nextLanguage);
    return nextLanguage;
  });
  ...
  return <TooltipProvider delayDuration={200}><div className="lab-site" data-theme={theme} ref={root}>
    ...
    <header className="x-nav"><a href="#top" className="x-logo"><WolfMark/><span>StudioWolfCode<small>FRONTEND / PRODUCT UI</small></span></a><nav className={menuOpen ? "open" : ""}>{(es ? [["Proyectos","work"],["Especialidad","expertise"],["Sobre mí","about"],["Lab","lab"],["Contacto","contact"]] : [["Work","work"],["Expertise","expertise"],["About","about"],["Lab","lab"],["Contact","contact"]]).map(([label,id]) => <a key={id} href={`#${id}`} onClick={() => setMenuOpen(false)}>{label}</a>)}<button className="command-trigger" onClick={() => setCommandOpen(true)}><CommandIcon size={13}/> {es ? "Navegar" : "Navigate"} <kbd>⌘K</kbd></button></nav><div className="nav-actions"><button className="language-toggle" type="button" onClick={toggleLanguage} aria-label={es ? "Switch to English" : "Cambiar a castellano"} title={es ? "English" : "Castellano"}><span className={!es ? "active" : ""}>EN</span><i/><span className={es ? "active" : ""}>ES</span></button><button className="theme-toggle" type="button" onClick={toggleTheme} aria-label={theme === "dark" ? (es ? "Usar tema claro" : "Use light theme") : (es ? "Usar tema oscuro" : "Use dark theme")} title={theme === "dark" ? (es ? "Tema claro" : "Light theme") : (es ? "Tema oscuro" : "Dark theme")}>{theme === "dark" ? <Sun/> : <Moon/>}</button><button className="x-menu" onClick={() => setMenuOpen(!menuOpen)} aria-expanded={menuOpen} aria-label={es ? "Abrir navegación" : "Toggle navigation"}>{menuOpen ? <X/> : <Menu/>}</button></div></header>
    <CommandPalette open={commandOpen} setOpen={setCommandOpen}/>
    <main>
      <section className="x-hero" id="top"><div className="hero-grid"/><div className="x-hero-copy"><Badge><i className="live-dot"/> {es ? "ABIERTO A OPORTUNIDADES FRONTEND / PRODUCT ENGINEERING" : "OPEN TO FRONTEND / PRODUCT ENGINEERING OPPORTUNITIES"}</Badge><p className="hero-name">{profile.headline}</p><h1>{es ? "Diseño productos." : "I design products."}<br/><span>{es ? "Construyo la interfaz." : "I build the interface."}</span></h1><p className="hero-lead">{es ? "Senior Frontend Engineer especializado en Product UI, Design Systems e interfaces emergentes con IA. Construyo productos SaaS y empresariales complejos con React, Next.js y TypeScript." : "Senior Frontend Engineer working across product UI, design systems and emerging AI interfaces. I build complex SaaS and enterprise products with React, Next.js and TypeScript."}</p><div className="hero-tech">{profile.stack.map(item => <span key={item}>{item}</span>)}</div><div className="hero-cta"><Button asChild><a href="#work">{es ? "Ver proyectos" : "View projects"} <ArrowRight/></a></Button><Button asChild variant="outline"><a href="#about">{es ? "Sobre mí" : "About me"} <ArrowRight/></a></Button></div><div className="hero-proof"><div><strong>7+</strong><span>{es ? <>Años alrededor de<br/>productos digitales</> : <>Years around<br/>digital products</>}</span></div><div><strong>UI</strong><span>SaaS · ERP<br/>CRM · Dashboards</span></div><div><strong>AI</strong><span>{es ? <>Desarrollando<br/>interfaces agénticas</> : <>Currently building<br/>agentic interfaces</>}</span></div></div></div><HeroComposition/><div className="scroll-cue"><span>{es ? "DESPLAZÁ PARA EXPLORAR" : "SCROLL TO EXPLORE"}</span><i/></div></section>
      ...
      <section className="x-expertise x-section" id="expertise"><SectionLabel number="02">{es ? "Lo que construyo / Cuatro disciplinas conectadas" : "What I Build / Four connected disciplines"}</SectionLabel><div className="x-section-head"><h2>{es ? "Ingeniería primero." : "Engineering first."}<br/><span>{es ? "Diseño como ventaja." : "Design as leverage."}</span></h2><p>{es ? "Trabajo donde se encuentran la arquitectura frontend, las decisiones de producto y los sistemas de interfaz." : "I work where frontend architecture, product decisions and interface systems meet."}</p></div><div className="pillar-system">{buildPillars.map((pillar) => <article key={pillar.id} className="pillar-row"><span>{pillar.id}</span><h3>{es ? ({"Product UI":"UI de Producto","Frontend Engineering":"Ingeniería Frontend","Design Systems":"Sistemas de Diseño","AI Interfaces":"Interfaces con IA"}[pillar.title] ?? pillar.title) : pillar.title}</h3><p>{es ? ({"01":"SaaS, ERP, CRM e interfaces con alta densidad de datos construidas para el trabajo operativo real.","02":"Interfaces React tipadas y responsive, con arquitectura de componentes y límites claros.","03":"Tokens, primitivas y patrones reutilizables que conectan decisiones de producto con código.","04":"Flujos agénticos emergentes con herramientas, aprobación, trazabilidad y recuperación visibles."}[pillar.id]) : pillar.copy}</p><code>{pillar.signal}</code></article>)}</div></section>
      ...
      <section className="x-design-system x-section" id="design-system"><SectionLabel number="04">{es ? "Sistemas de Diseño / De fundamentos a producto" : "Design Systems / From foundations to product"}</SectionLabel><div className="system-showcase"><div className="system-copy"><Badge>{es ? "MUESTRA DE COMPONENTES" : "LIVE COMPONENT SPECIMEN"}</Badge><h2>{es ? "No es una lista." : "Not a list."}<br/><span>{es ? "Es un sistema real." : "A working system."}</span></h2><p>{es ? "Los fundamentos se convierten en tokens, primitivas y patrones de producto repetibles. La misma arquitectura sostiene dashboards, formularios, tablas y estados de IA." : "Foundations become tokens, primitives and repeatable product patterns. The same architecture supports dashboards, forms, tables and AI states."}</p><div className="architecture-flow">{(es ? ["Fundamentos","Tokens","Primitivas","Componentes","Patrones","Funcionalidades"] : ["Foundations","Tokens","Primitives","Components","Patterns","Features"]).map((item, index) => <span key={item}>{item}{index < 5 && <i>→</i>}</span>)}</div></div><Card className="component-specimen"><div className="specimen-header"><span>{es ? "componentes / estados interactivos" : "components / interactive states"}</span><Badge>v1.4</Badge></div><div className="token-strip"><i/><i/><i/><i/><code>space.04 / radius.02 / accent.500</code></div><div className="specimen-controls"><Button>{es ? "Acción principal" : "Primary action"}</Button><Button variant="outline">{es ? "Secundaria" : "Secondary"}</Button><Badge>{es ? "ACTIVO" : "ACTIVE"}</Badge></div><label className="specimen-input"><span>{es ? "Nombre del espacio" : "Workspace name"}</span><input defaultValue="Operations Suite" aria-label={es ? "Nombre del espacio" : "Workspace name"}/></label><div className="specimen-row"><span><i className="live-dot"/> {es ? "Listo para desplegar" : "Deployment ready"}</span><strong>{es ? "Revisar cambios" : "Review changes"}</strong><small>⌘ Enter</small></div></Card></div></section>

      <AiLab es={es}/>

      <section className="x-about x-section" id="about"><SectionLabel number="06">{es ? "Sobre mí / Un recorrido de diseño que termina en ingeniería" : "About / A design background that ends in engineering"}</SectionLabel><div className="about-grid"><div className="identity-visual x-reveal"><div className="portrait-grid"/><div className="portrait-monogram">SG</div><div className="portrait-code"><code>role: <span>&quot;frontend engineer&quot;</span><br/>method: <span>&quot;product-minded&quot;</span><br/>focus: <span>&quot;systems + AI UI&quot;</span></code></div><Badge>BUENOS AIRES · UTC−3</Badge></div><div className="about-content x-reveal"><Badge>FRONTEND ENGINEERING + PRODUCT UI</Badge><h2>{es ? "El diseño me enseñó a ver." : "Design taught me how to see."} <span>{es ? "La ingeniería me permite construir." : "Engineering lets me ship."}</span></h2><p className="about-lead">{es ? "Soy Frontend Engineer con una base profunda en Product UI." : "I am a frontend engineer with a deep Product UI background."}</p><p>{es ? "Mi recorrido pasó del diseño gráfico a UI, Product Design y Frontend Engineering. Esa formación visual hoy es una ventaja: puedo pensar el producto, definir su sistema de interfaz y construirlo en código." : "My path moved from graphic design to UI, product design and frontend engineering. That visual training is now leverage: I can reason about the product, define its interface system and build it in code."}</p><div className="fact-grid"><div><strong>7+</strong><span>{es ? "Años alrededor de producto" : "Years around products"}</span></div><div><strong>R / TS</strong><span>React · TypeScript</span></div><div><strong>3×</strong><span>SaaS · ERP · CRM</span></div><div><strong>AI</strong><span>{es ? "Aprendizaje aplicado" : "Applied learning"}</span></div></div><div className="about-links"><a href={profile.linkedin} target="_blank" rel="noreferrer"><Linkedin/> LinkedIn</a></div></div></div></section>
      ...
      <section className="x-contact" id="contact"><div className="contact-grid-bg"/><Badge><i className="live-dot"/> {es ? "ABIERTO A OPORTUNIDADES FRONTEND, PRODUCT ENGINEERING E INTERFACES CON IA" : "OPEN TO FRONTEND, PRODUCT ENGINEERING AND AI INTERFACE WORK"}</Badge><h2>{es ? "Construyamos" : "Let’s build"}<br/><span>{es ? "algo útil." : "something useful."}</span></h2><p>{es ? "Product UI complejo, un rol de ingeniería frontend o un flujo con IA que valga la pena hacer claro." : "Complex product UI, a frontend engineering role, or an AI workflow worth making clear."}</p><div className="contact-actions"><Button asChild><a href={profile.linkedin} target="_blank" rel="noreferrer"><Linkedin/> {es ? "Conectar en LinkedIn" : "Connect on LinkedIn"}</a></Button><Button variant="outline" onClick={copyContact}>{copied ? <Check/> : <Copy/>}{copied ? (es ? "Enlace copiado" : "Profile link copied") : (es ? "Copiar LinkedIn" : "Copy LinkedIn")}</Button></div><div className="contact-footer"><a href="#top" className="x-logo"><WolfMark/><span>StudioWolfCode</span></a><p>Frontend Engineering · Product UI · Design Systems · AI Interfaces</p><span>© {new Date().getFullYear()} {profile.name}</span></div></section>
    </main>
  </div></TooltipProvider>;
}
